...
%   - Soil loss: daily sediment delivery (kg/m)
% - Input (predictor) variable:
%   - A hillslope profile (`slp` file)
%     - Slope: slope steepness (m/m) at a point
%     - Normalized distance: distance [0-1] from the top of the hillslope to the point
%     - Mean slope: mean slope (m/m) over the profile
%     - Hillslope length: length (m) of the overland flow element
%   - A climate profile (`cli` file)
%   - Precipitation: daily precipitation amount (mm of water)
  ...
  $\mathrm{Cor}(y_i, y_j) =
  \underbrace{\exp\{{(\omega(u)(X_i(u) - X_j(t)))}^2\mathrm{d}u\}}_{\mathrm{ADRD}}
  \times
  \underbrace{
